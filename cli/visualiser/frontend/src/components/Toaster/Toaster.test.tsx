...
describe("Toaster", () => {
      ...
      toasts: [
        {
          ...
          heading: "External edit detected",
          message:
            "`meta/work/0007-foo.md` was updated while you were looking at it.",
          kind: "info",
        },
      ],
    });
  ...
  it("Escape during an active drag does NOT dismiss (dnd-kit owns drag-cancel)", () => {
    ...
    const handle = mockToastState({
      toasts: [
        { id: 9, heading: "Move failed", message: "oops", kind: "error" },
      ],
    });
    render(<Toaster />);
    fireEvent.keyDown(document, { key: "Escape" });
  ...
  it('routes an error toast to the assertive region with data-kind="error" and the error icon', () => {
    mockToastState({
      toasts: [
        { id: 1, heading: "Move failed", message: "conflict", kind: "error" },
      ],
    });
    render(<Toaster />);
    const assertive = screen.getByTestId("toaster-region-assertive");
    const card = within(assertive)
      .getByText("Move failed")
      .closest("[data-kind]")!;
    ...
    expect(
      within(screen.getByTestId("toaster-region-polite")).queryByTestId(
        "toaster-icon",
      ),
    ).toBeNull();
  });
  ...
  it("omits the message paragraph for a heading-only (empty message) toast", () => {
    mockToastState({
      toasts: [
        {
          id: 1,
          heading: "0086 moved to In progress",
          message: "",
          kind: "ok",
        },
      ],
    ...
    render(<Toaster />);
    const card = screen
      .getByText("0086 moved to In progress")
      .closest("[data-kind]")!;
  ...
  it("an error toast is dismissable via the close button and via Escape (no drag)", () => {
    const handle = mockToastState({
      toasts: [
        { id: 5, heading: "Move failed", message: "conflict", kind: "error" },
      ],
    });
    render(<Toaster />);
    fireEvent.click(
  ...
  describe("CSS source assertions", () => {
    it(".toast binds var(--ac-bg-card)", () => {
      expect(toasterCss).toMatch(
        /\.toast[^{]*\{[^}]*background[^}]*var\(--ac-bg-card\)/,
      );
    });

    it(".toast binds var(--ac-shadow-lift)", () => {
      expect(toasterCss).toMatch(
        /\.toast[^{]*\{[^}]*box-shadow[^}]*var\(--ac-shadow-lift\)/,
      );
    });
    ...
    it(".icon and border-left bind var(--toast-accent), defaulting to --ac-accent", () => {
      ...
      expect(toasterCss).toMatch(
        /\.icon[^{]*\{[^}]*color:\s*var\(--toast-accent\)/,
      );
      expect(toasterCss).toMatch(
        /\.toast[^{]*\{[^}]*border-left[^;]*var\(--toast-accent\)/,
      );
    ...
    it("data-kind='ok'/'error' remap --toast-accent to --ac-ok / --ac-err", () => {
      expect(toasterCss).toMatch(
        /\[data-kind='ok'\][^{]*\{[^}]*--toast-accent:\s*var\(--ac-ok\)/,
      );
      expect(toasterCss).toMatch(
        /\[data-kind='error'\][^{]*\{[^}]*--toast-accent:\s*var\(--ac-err\)/,
      );
    });
  });
});
