...
const CHIP_SELECTOR =
  '[data-testid="status-badge"],[data-testid="frontmatter-chip"]';
...
describe("FrontmatterChips", () => {
  describe("parsed state", () => {
    ...
    it("skips null and undefined values", () => {
      const { container } = render(
        <FrontmatterChips
          state="parsed"
          frontmatter={
            { status: "draft", author: null, date: undefined } as Record<
              string,
              unknown
            >
          }
    ...
    it("skips empty-string values", () => {
      const { container } = render(
        <FrontmatterChips
          state="parsed"
          frontmatter={{ status: "draft", author: "" }}
        />,
      );
      const chips = container.querySelectorAll(CHIP_SELECTOR);
      expect(chips.length).toBe(1);
    ...
    it("renders a chip for a non-string canonical value (smoke test)", () => {
      ...
      expect(labels).toEqual(["date: 3d ago", "author: Alice, Bob"]);
    });
  });
  ...
  describe("zero qualifying keys (parsed state)", () => {
    it("renders the empty chip-strip container when no canonical keys qualify", () => {
      const { container } = render(
        <FrontmatterChips
          state="parsed"
          frontmatter={{ priority: "medium", tags: ["x"] }}
        />,
      );
  ...
  describe("dispatch", () => {
    ...
    it("dispatches non-tone keys to FrontmatterChip", () => {
      ...
      expect(
        container.querySelectorAll('[data-testid="frontmatter-chip"]').length,
      ).toBe(2);
      expect(
        container.querySelector('[data-testid="status-badge"]'),
      ).toBeNull();
    ...
    it.each([
      ...
      expect(
        container.querySelector(`[data-testid="${expectedComponent}"]`),
      ).not.toBeNull();
    });
  });
  ...
  describe("whitelist + canonical order", () => {
    ...
    it("renders at most three chips from {status, date, author}", () => {
      ...
      expect(chips.length).toBe(3);
      expect(
        Array.from(chips).map((el) => el.getAttribute("data-testid")),
      ).toEqual(["status-badge", "frontmatter-chip", "frontmatter-chip"]);
    });
  ...
  describe("whitelist exclusion", () => {
    it.each([
      ...
      const { container } = render(
        <FrontmatterChips
          state="parsed"
          frontmatter={{ [key]: value } as Record<string, unknown>}
        />,
      );
      ...
      expect(
        container.querySelector('[data-testid="frontmatter-chip"]'),
      ).toBeNull();
    });
  });
  ...
  describe("subset ordering", () => {
    it.each([
      [
        ...
        ["status: Draft", "date: 3d ago"],
      ],
      [
        { status: "draft", author: "Toby Clemson" },
        ["status: Draft", "author: Toby Clemson"],
      ],
      [
        { date: "2026-04-05", author: "Toby Clemson" },
        ["date: 3d ago", "author: Toby Clemson"],
      ],
      [
        { author: "Toby Clemson", date: "2026-04-05" },
        ["date: 3d ago", "author: Toby Clemson"],
      ],
    ...
    it("skipped (null) case-variant does not block a later valid canonical key", () => {
      const { container } = render(
        <FrontmatterChips
          state="parsed"
          frontmatter={
            { Status: null, status: "draft" } as Record<string, unknown>
          }
        />,
      );
      const labels = Array.from(container.querySelectorAll("[aria-label]")).map(
    ...
    it("first non-skipped value wins among case-variant duplicates", () => {
      const { container } = render(
        <FrontmatterChips
          state="parsed"
          frontmatter={
            { Status: "first", status: "second" } as Record<string, unknown>
          }
        />,
      );
      const labels = Array.from(container.querySelectorAll("[aria-label]")).map(
...
describe("per-doc-kind corpus verification", () => {
  ...
  it.each(
    DOC_TYPE_KEYS,
  )('kind "%s" never renders the extra non-canonical key as a chip', (kind) => {
