...
describe("Popover", () => {
  ...
  it("panel is hidden when closed and visible when open", async () => {
    ...
    expect(
      document.querySelector('[role="menu"]')!.hasAttribute("hidden"),
    ).toBe(false);
  });
  ...
  it("positions the panel via CSS (top below trigger, right-aligned)", () => {
    ...
    expect(popoverCss).toMatch(
      /\.panel\s*\{[^}]*top:\s*calc\(100%\s*\+\s*var\(--sp-1\)\)/,
    );
  ...
  it("Enter activates the focused item", async () => {
    ...
    function WithClick() {
      ...
      return (
        ...
        >
          {/* biome-ignore lint/a11y/useKeyWithClickEvents: fixture proving the Popover routes keyboard activation to the focused item's click handler; the keyboard handling under test lives in Popover, not on this item */}
          <div role="menuitem" tabIndex={-1} onClick={onItemClick}>
            A
          </div>
        </Popover>
      );
