...
describe("parseRgba", () => {
  ...
  it("parses rgba() with fractional alpha", () => {
    expect(parseRgba("rgba(0, 0, 0, 0.5)")).toEqual({
      r: 0,
      g: 0,
      b: 0,
      a: 0.5,
  ...
  it("tolerates whitespace and integer alpha", () => {
    expect(parseRgba("rgba( 16, 32, 64, 1 )")).toEqual({
      r: 16,
      g: 32,
      b: 64,
      a: 1,
...
describe("composeOverSurface", () => {
  it("returns the surface when foreground alpha is 0", () => {
    expect(
      composeOverSurface("rgba(255, 255, 255, 0)", "#000000").toLowerCase(),
    ).toBe("#000000");
  });
  it("returns the foreground when alpha is 1", () => {
    expect(
      composeOverSurface("rgba(128, 128, 128, 1)", "#000000").toLowerCase(),
    ).toBe("#808080");
  });
  it("blends 50% black over white to mid-grey", () => {
    expect(
      composeOverSurface("rgba(0, 0, 0, 0.5)", "#ffffff").toLowerCase(),
    ).toBe("#808080");
  });
  it("accepts hex foreground (treated as alpha=1)", () => {
    expect(composeOverSurface("#cc0000", "#ffffff").toLowerCase()).toBe(
      "#cc0000",
    );
  });
});
...
describe("contrastRatioComposed", () => {
  it("composes 50% black over white and contrasts against white", () => {
    ...
    expect(
      contrastRatioComposed("rgba(0, 0, 0, 0.5)", "#ffffff", "#ffffff"),
    ).toBeCloseTo(3.95, 1);
  });
  it("opaque hex foreground passes through composeOverSurface unchanged", () => {
    expect(contrastRatioComposed("#000000", "#ffffff", "#ffffff")).toBeCloseTo(
      21,
      1,
    );
  });
...
describe("design-token contrast (WCAG 2.2 AA, light)", () => {
  it("--ac-fg on --ac-bg ≥ 4.5:1", () => {
    expect(
      contrastRatio(LIGHT_COLOR_TOKENS["ac-fg"], lightBg),
    ).toBeGreaterThanOrEqual(4.5);
  });
  it("--ac-fg-muted on --ac-bg ≥ 4.5:1", () => {
    expect(
      contrastRatio(LIGHT_COLOR_TOKENS["ac-fg-muted"], lightBg),
    ).toBeGreaterThanOrEqual(4.5);
  });
  it("--ac-accent on --ac-bg ≥ 3:1 (UI component, WCAG 1.4.11)", () => {
    expect(
      contrastRatio(LIGHT_COLOR_TOKENS["ac-accent"], lightBg),
  ...
  it("composed --ac-stroke-strong over --ac-bg produces a visible tint (> 1:1)", () => {
    ...
    expect(
      contrastRatioComposed(
        LIGHT_COLOR_TOKENS["ac-stroke-strong"],
        lightBg,
        lightBg,
      ),
    ).toBeGreaterThan(1);
  ...
  it("--ac-warn is darker than a 12%-warn-tinted bg (ratio ≥ 2:1)", () => {
    ...
    expect(
      contrastRatio(LIGHT_COLOR_TOKENS["ac-warn"], warnBg),
...
describe("design-token contrast (WCAG 2.2 AA, dark)", () => {
  it("--ac-fg on --ac-bg ≥ 4.5:1", () => {
    expect(
      contrastRatio(DARK_COLOR_TOKENS["ac-fg"], darkBg),
    ).toBeGreaterThanOrEqual(4.5);
  });
  it("--ac-fg-muted on --ac-bg ≥ 4.5:1", () => {
    expect(
      contrastRatio(DARK_COLOR_TOKENS["ac-fg-muted"], darkBg),
    ).toBeGreaterThanOrEqual(4.5);
  });
  it("--ac-accent on --ac-bg ≥ 3:1", () => {
    expect(
      contrastRatio(DARK_COLOR_TOKENS["ac-accent"], darkBg),
    ).toBeGreaterThanOrEqual(3);
  });
  ...
  it("--ac-warn (theme-invariant) on dark --ac-bg ≥ 3:1", () => {
    expect(
      contrastRatio(LIGHT_COLOR_TOKENS["ac-warn"], darkBg),
    ).toBeGreaterThanOrEqual(3);
  });
  it("--ac-err (theme-invariant) on dark --ac-bg ≥ 3:1", () => {
    expect(
      contrastRatio(LIGHT_COLOR_TOKENS["ac-err"], darkBg),
    ).toBeGreaterThanOrEqual(3);
  });
  it("--ac-ok (theme-invariant) on dark --ac-bg ≥ 3:1", () => {
    expect(
      contrastRatio(LIGHT_COLOR_TOKENS["ac-ok"], darkBg),
    ).toBeGreaterThanOrEqual(3);
  });
  it("--ac-violet (theme-invariant) on dark --ac-bg ≥ 3:1", () => {
    expect(
      contrastRatio(LIGHT_COLOR_TOKENS["ac-violet"], darkBg),
    ).toBeGreaterThanOrEqual(3);
  });
});
