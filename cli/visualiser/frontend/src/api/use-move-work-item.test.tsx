...
describe("useMoveWorkItem", () => {
  ...
  it("optimistically updates status in cache", async () => {
    ...
    vi.spyOn(fetchModule, "patchWorkItemFrontmatter").mockReturnValue(
      new Promise(() => {}),
    );

    const { result } = renderHook(() => useMoveWorkItem(), { wrapper });
    act(() => {
      result.current.mutate({ entry, toStatus: "in-progress" });
    });

    await waitFor(() => {
      const cached = queryClient.getQueryData<IndexEntry[]>(
  ...
  it("registers self etag on success", async () => {
    ...
    vi.spyOn(fetchModule, "patchWorkItemFrontmatter").mockResolvedValue({
      etag: "sha256-NEW",
    });
  ...
  it("does not modify other entries in cache", async () => {
    ...
    const { result } = renderHook(() => useMoveWorkItem(), { wrapper });
    act(() => {
      result.current.mutate({ entry: entryA, toStatus: "in-progress" });
    });
