...
describe("LibraryDocView", () => {
  ...
  it("renders error path with role=alert when fetchRelated fails", async () => {
    ...
    vi.spyOn(fetchModule, "fetchRelated").mockRejectedValue(
      new Error("related-boom"),
    );
    render(<LibraryDocView type="plans" fileSlug="2026-01-01-foo" />, {
      wrapper: Wrapper,
    });
  ...
  it("real wiring resolves wiki-link to anchor when ADR is in cache", async () => {
    ...
    expect(link.getAttribute("href")).toBe(
      "/library/decisions/ADR-0001-example",
    );
  ...
  it("renders a malformed-frontmatter banner when entry.frontmatterState is malformed", async () => {
    ...
    expect(banner).toHaveTextContent(
      /We couldn.t read this document.s metadata header/i,
    );
  ...
  it("linkifies a WORK-ITEM scalar value via the shared resolver (end-to-end)", async () => {
    ...
    const { container } = render(
      <LibraryDocView
        type="work-items"
        fileSlug="0078-detail-page-frontmatter-table"
      />,
      { wrapper: Wrapper },
    );
  ...
  it("shows the Cluster heading + Loading while the cluster query is pending", async () => {
    ...
    const clusterHeading = await screen.findByRole("heading", {
      name: "Cluster",
    });
  ...
  it("shows a role=alert in the Cluster section when fetchLifecycleClusters rejects", async () => {
    ...
    expect(
      await screen.findByText(/Could not load cluster/i),
    ).toBeInTheDocument();
  ...
  it("renders both header actions in the actions slot for a resolved doc", async () => {
    ...
    expect(
      within(actions as HTMLElement).getByRole("button", { name: "Copy path" }),
    ).toBeInTheDocument();
    expect(
