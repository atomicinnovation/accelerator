...
import { FetchError, fetchDocs } from "../../api/fetch";
...
import { ActivityIcon, KanbanEyebrowIcon } from "./icons";
import styles from "./KanbanBoard.module.css";
import { KanbanColumn } from "./KanbanColumn";
import {
  createKanbanFocusRegistry,
  KanbanFocusContext,
} from "./kanban-focus-registry";
...
export function KanbanBoard() {
  ...
  const groups = useMemo(
    () => groupWorkItemsByStatus(entries, columns),
    [entries, columns],
  );
  ...
  useEffect(() => {
    entriesRef.current = entriesByRelPath;
  }, [entriesByRelPath]);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: `entries` is an intentional re-run trigger, not a captured value — the effect must fire each time the entries list re-renders (e.g. after an SSE refetch remount) to re-apply the pending focus token; removing it would break focus-after-refetch
  useEffect(() => {
  ...
  function handleDragEnd({ active, over }: DragEndEvent) {
    ...
    switch (outcome.kind) {
      case "move": {
        ...
        const targetLabel = columns.find(
          (c) => c.key === outcome.toStatus,
        )?.label;
        ...
        move.mutate(
          ...
          {
            onSuccess: () => {
              const toast = moveToastFor(outcome, source, targetLabel, {
                ok: true,
              });
  ...
  return (
    <Page
      ...
      actions={
        isSuccess ? (
          <>
            <Chip variant="indigo" leading={<ActivityIcon />}>
              live
            </Chip>
