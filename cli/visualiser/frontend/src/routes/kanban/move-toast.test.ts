...
describe("moveToastFor", () => {
  ...
  it("heading names the card via the shared describeEntry helper", () => {
    ...
    expect(toast.heading).toBe(
      "work item 0086: Kanban DnD moved to In progress",
    );
  });
  ...
  it("any non-move outcome returns null (no toast)", () => {
    expect(
      moveToastFor({ kind: "no-op-same-column" }, entry, "In progress", {
        ok: true,
      }),
    ).toBeNull();
    expect(
      moveToastFor({ kind: "no-op-other-rejected" }, entry, "In progress", {
        ok: true,
      }),
    ).toBeNull();
    expect(
      moveToastFor({ kind: "no-op-unknown" }, entry, "In progress", {
        ok: true,
      }),
    ).toBeNull();
  });
  ...
  it("a missing source entry (deleted mid-drag) returns null even for a move", () => {
    expect(
      moveToastFor(MOVE, undefined, "In progress", { ok: true }),
    ).toBeNull();
  });
});
