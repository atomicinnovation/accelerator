...
for (const { name, columns, target, targetLabel } of [
  ...
  {
    name: "5-column (wrapping label)",
    columns: FIVE_COLUMNS,
    target: "review",
    targetLabel: LONG_LABEL,
  },
] as const) {
  test.describe(`kanban ${name} config`, () => {
    test.beforeEach(async ({ page }) => {
      await page.setViewportSize({ width: 1024, height: 800 });
      await installMockBoard(page, {
        columns: [...columns],
        cards: [CARD_FIXTURE],
      });
    });
    ...
    test("drag + drop raises a success toast and the card stays", async ({
      page,
    }) => {
      await page.goto("/kanban");
      await expect(
      ...
      await expect(
        page.locator(`section[data-column="${target}"] ${CARD}`),
      ).toBeVisible({
        timeout: 5000,
      });
      await expect(
...
test("the ≥30-char column label wraps to ≥2 lines without breaking the drop target", async ({
  ...
  await expect(
    reviewColumn.getByRole("heading", { name: LONG_LABEL }),
  ).toBeVisible();
});
