...
test.describe("Regression — legacy-schema work-items", () => {
  test("legacy work-item with non-configured status lands in the Other swimlane", async ({
    page,
  }) => {
  ...
  test("library view of a legacy-type work-item renders without errors", async ({
    page,
  }) => {
  ...
  test("PATCH from Other (non-configured status) to a configured column succeeds", async ({
    ...
    try {
      ...
      const patchRes = await request.patch(
        `/api/docs/${LEGACY_OTHER_REL}/frontmatter`,
        {
  ...
  test("PATCH with an unknown status returns 400 even for legacy files", async ({
    request,
  }) => {
    const docsRes = await request.get("/api/docs?type=work-items");
    const { docs } = (await docsRes.json()) as {
