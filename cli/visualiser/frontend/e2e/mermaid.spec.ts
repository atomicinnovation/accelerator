...
test("mermaid code block is rendered (not shown as raw syntax)", async ({
  page,
}) => {
  ...
  const mermaidSvg = page
    .locator("svg")
    .filter({ has: page.locator('[class*="mermaid"]') });
  ...
  if (hasSvg) {
    ...
    throw new Error(
      "Expected mermaid block to appear as either SVG or code element",
    );
  }
});
