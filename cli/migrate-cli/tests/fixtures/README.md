...
- `interactive/{doc-example,accept-verb,three-decision,validator-rejecting,unowned-dirty-path,two-owned-dirty-paths}/` — scripted decision sequences and dirty-tree resume/refusal cases, adapted from the historical `test-migrate-interactive.sh` test suite's own helpers.
