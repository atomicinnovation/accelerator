...
pub(crate) mod test_support {
    ...
    // ── Parallel-safe log capture ──────────────────────────────────────────
    // `with_default` (thread-local) capture is unreliable for warnings emitted
    // from production callsites that OTHER parallel tests also hit: tracing's
    // callsite-interest cache is global, so a no-subscriber test that reaches
    // the callsite first caches its interest as "never", and the capturing
    // thread's event is then skipped. We instead install a permissive GLOBAL
    // subscriber once (so every callsite's interest stays "always") and route
    // its output to a per-thread buffer. Each test captures only the events its
    // own thread emits.
    ...
    /// Run `f` with all tracing output its thread emits captured; return it as
    /// a String. Safe under parallel test execution.
    pub fn capture_logs<F: FnOnce()>(f: F) -> String {
        CAPTURE_INIT.call_once(|| {
            let subscriber = tracing_subscriber::fmt()
                ...
                .finish();
            // Ignore the error if a global default is already installed — as
            // long as it is this permissive one, capture still works.
            let _ = tracing::subscriber::set_global_default(subscriber);
        });
        let buf = Arc::new(Mutex::new(Vec::<u8>::new()));
        CAPTURE_BUF.with(|c| *c.borrow_mut() = Some(buf.clone()));
        f();
        CAPTURE_BUF.with(|c| *c.borrow_mut() = None);
