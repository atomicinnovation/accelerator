...
import type { ActionKind, IndexEntry, SseEvent } from "../../api/types";
import {
  DocEventsContext,
...
function renderSubstituteTree(
  atUrl: string,
  seedEntries: IndexEntry[],
): Harness {
...
describe("RootLayout external-edit integration (substitute tree)", () => {
  ...
  async function expectExternalEditToast(verb: string): Promise<void> {
    expect(
      await screen.findByText("External edit detected"),
    ).toBeInTheDocument();
  ...
  it('positive correlation: doc-changed action=edited raises "updated" toast', async () => {
    const harness = renderSubstituteTree(`/library/work-items/${SLUG}`, [
      entry(),
    ]);
  ...
  it("auto-dismiss + pause-on-hover end-to-end (fake timers)", async () => {
    ...
    try {
      ...
      expect(screen.getByText("External edit detected")).toBeInTheDocument();
      const card = screen.getByText("External edit detected").closest("div")!
        .parentElement!;
