...
function makeUnseenHandle(
  unseenSet: Set<DocTypeKey> = new Set(),
): UnseenDocTypesHandle {
  return {
    unseenSet,
    markSeen: vi.fn(),
    onEvent: vi.fn(),
...
const allDocTypes: DocType[] = [
  ...
  {
    key: "work-item-reviews",
    label: "Work item reviews",
    dirPath: "/p",
    inLifecycle: true,
    inKanban: false,
    virtual: false,
  ...
  {
    key: "design-inventories",
    label: "Design inventories",
    dirPath: "/p",
    inLifecycle: true,
    inKanban: false,
    virtual: false,
...
function defaultPhases(docTypes: DocType[]): LibraryPhase[] {
  ...
  const phases: { id: string; label: string; ids: DocTypeKey[] }[] = [
    ...
    },
    {
      id: "build",
      label: "Build",
      ids: ["plans", "plan-reviews", "validations"],
    },
...
describe("Sidebar", () => {
  ...
  it("does not render glyphs in LIBRARY nav items", async () => {
    ...
    const svgs = container.querySelectorAll(
      'section[aria-labelledby="library-heading"] svg',
    );
  ...
  it("VIEWS section renders Kanban and Lifecycle with icons", async () => {
    ...
    const svgs = container.querySelectorAll(
      'section[aria-labelledby="views-heading"] svg',
    );
  ...
  it("unseen dot present when context flags the type", async () => {
    ...
    expect(
      researchLink.querySelectorAll('span[aria-hidden="true"]').length,
    ).toBe(1);
    expect(researchLink.getAttribute("title")).toBe(
      "Unseen changes since your last visit",
    );
    const decisionsLink = screen.getByLabelText("Decisions");
    expect(
      decisionsLink.querySelectorAll('span[aria-hidden="true"]').length,
    ).toBe(0);
  });
  ...
  describe("Activity slot", () => {
    ...
    it("renders the ACTIVITY section between VIEWS and META in DOM order", async () => {
      ...
      expect(
        views.compareDocumentPosition(activity) &
          Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
      expect(
        activity.compareDocumentPosition(meta) &
          Node.DOCUMENT_POSITION_FOLLOWING,
      ).toBeTruthy();
    });
