...
import { BigGlyph, BIG_GLYPHS, DEFAULT_BIG_HUE } from './BigGlyph'
...
describe('bigPalette', () => {
  it('returns exactly seven tones with white fixed to #ffffff', () => {
    ...
    expect(Object.keys(p).sort()).toEqual(
      ['accent', 'deep', 'fill', 'fold', 'line', 'stroke', 'white'],
    )
  ...
  it.each([0, 12, 215, 280])(
    'derives all six hue tones from the input hue %i',
    (hue) => {
...
describe('BigGlyph: hue resolution', () => {
  ...
  it('an explicit hue prop overrides the doc-type hue', () => {
    ...
    const svg = container.querySelector('svg')!
    expect(collectColours(svg).some((c) => c.startsWith('hsl(280 '))).toBe(true)
  })
...
describe('BigGlyph: source-walk literal guard', () => {
  ...
  it.each(DOC_TYPE_KEYS)(
    'uses only sanctioned colours in the %s illustration',
    (docType) => {
