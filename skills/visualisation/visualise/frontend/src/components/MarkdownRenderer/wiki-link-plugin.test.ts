...
describe("remarkWikiLinks: rewrites resolved ref to a link node with title as display text", () => {
  it('produces [Text("see "), Link(title-as-text, hProperties.title=bracketForm)]', () => {
    ...
    expect(link.children).toEqual([
      { type: "text", value: "Example decision" },
    ]);
...
describe("remarkWikiLinks: emits unresolved marker when resolver returns kind=unresolved", () => {
  it('produces [Text("see "), MarkerNode(unresolved-wiki-link, diagnostic title)]', () => {
    ...
    const marker = para.children[1] as {
      ...
      data: {
        hName: string;
        hProperties: { className: string; title: string };
    ...
    expect(marker.data.hChildren).toEqual([
      { type: "text", value: "[[ADR-9999]]" },
    ]);
  });
});
  ...
  it("rewrites bare-numeric [[WORK-ITEM-0007]] under a project pattern (legacy fallback)", () => {
    ...
    const resolver: Resolver = (prefix, id) =>
      prefix === "WORK-ITEM" && id === "0007"
        ? {
            kind: "resolved",
            href: "/library/work-items/0007-bar",
            title: "Bar",
          }
        : { kind: "unresolved" };
