...
describe("FrontmatterChip", () => {
  describe("default rendering", () => {
    ...
    // biome-ignore lint/suspicious/noTemplateCurlyInString: literal naming the aria-label format, not a template
  ...
  describe("value formatting", () => {
    it('joins array values with ", " (and the join appears in aria-label)', () => {
      ...
      expect(
        container.querySelector('[aria-label="tags: design, frontend"]'),
      ).not.toBeNull();
    });

    it("JSON-stringifies plain objects (visible text and aria-label parity)", () => {
      const { container } = render(
        <FrontmatterChip name="meta" value={{ x: 1 }} />,
      );
      expect(screen.getByText('{"x":1}')).toBeInTheDocument();
      expect(
        container.querySelector('[aria-label="meta: {\\"x\\":1}"]'),
      ).not.toBeNull();
    });
