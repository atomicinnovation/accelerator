...
import { buildEditorHref, EDITOR_PRESETS, encodePath } from "./editor-link";
...
function href(
  editor: string,
  opts?: { abs?: string; rel?: string; project?: string },
) {
...
describe("buildEditorHref — preset table", () => {
  ...
  it("covers exactly the documented preset keys (no drift)", () => {
    expect(Object.keys(EDITOR_PRESETS).sort()).toEqual(
      [...VSCODE, ...JETBRAINS].sort(),
    );
  });

  it.each(
    VSCODE,
  )("VS Code family %s → {scheme}://file{abs} (single slash)", (key) => {
    expect(href(key)).toBe(`${key}://file/Users/x/a%20b.md`);
  });

  it.each(
    JETBRAINS,
  )("JetBrains %s → navigate/reference with tag, project, rel", (key) => {
...
describe("buildEditorHref — custom templates", () => {
  it("substitutes {abs} verbatim: zed://file{abs}", () => {
    expect(href("zed://file{abs}", { abs: "/a b/c.md" })).toBe(
      "zed://file/a%20b/c.md",
    );
  });
...
describe("buildEditorHref — unresolvable / unsafe → null", () => {
  ...
  it.each([
    ...
    "blob",
    "file",
  ])("returns null for a dangerous %s: scheme even with a placeholder", (scheme) => {
