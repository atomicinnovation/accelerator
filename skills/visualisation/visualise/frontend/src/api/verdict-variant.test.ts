...
import { __SETS_FOR_TEST, verdictToVariant } from "./verdict-variant";
...
describe("verdictToVariant", () => {
  ...
  describe("neutral fallback", () => {
    it.each(["xyz", "", "undecided", "maybe"])("unmapped %s → neutral", (v) =>
      expect(verdictToVariant(v)).toBe("neutral"));
    ...
    it.each([
      ...
      ["a"],
      { x: 1 },
    ] as const)("non-string %s → neutral", (v) =>
      expect(verdictToVariant(v as unknown)).toBe("neutral"));
  });
