...
describe("groupWorkItemsByStatus", () => {
  it("groups by canonical status values", () => {
    ...
    const c = makeIndexEntry({
      relPath: "meta/work/0003-c.md",
  ...
  it('places work items with absent or malformed frontmatter in "other"', () => {
    const absent = makeIndexEntry({
      frontmatterState: "absent",
      frontmatter: {},
    });
    const malformed = makeIndexEntry({
  ...
  it("sorts each group by mtimeMs descending", () => {
    ...
    const mid = makeIndexEntry({
      relPath: "meta/work/0002-mid.md",
      frontmatter: { status: "todo" },
      mtimeMs: 200,
    });
    const newest = makeIndexEntry({
      relPath: "meta/work/0003-new.md",
      frontmatter: { status: "todo" },
      mtimeMs: 300,
    });
