...
function stubKanbanConfig(
  colsPayload: typeof THREE_COLS | typeof SEVEN_COLS = SEVEN_COLS,
) {
  vi.stubGlobal(
    "fetch",
    vi.fn((url: string) => {
...
describe("KanbanBoard", () => {
  ...
  it("renders the page-level heading at the top of the board", async () => {
    ...
    expect(
      await screen.findByRole("heading", { level: 1, name: /^work items$/i }),
    ).toBeInTheDocument();
  });
  ...
  it("renders the configured column set from the server", async () => {
    ...
    renderKanbanAt();
    expect(
      await screen.findByRole("region", { name: /ready/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: /in progress/i }),
    ).toBeInTheDocument();
  ...
  it("falls back gracefully if config endpoint returns the seven defaults", async () => {
    ...
    expect(screen.getByRole("region", { name: /^done$/i })).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: /blocked/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("region", { name: /abandoned/i }),
    ).toBeInTheDocument();
  });
  ...
  it("places work items in the column matching their frontmatter.status", async () => {
    ...
    expect(
      within(screen.getByRole("region", { name: /in progress/i })).getByText(
        "WIP work item",
      ),
    ).toBeInTheDocument();
    expect(
      within(screen.getByRole("region", { name: /done/i })).getByText(
        "Done work item",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("renders the Other swimlane with non-canonical statuses", async () => {
    const blocked = makeIndexEntry({
      type: "work-items",
      relPath: "meta/work/0007-blocked.md",
  ...
  it("renders a Retry button inside the error alert that invalidates the query", async () => {
    ...
    expect(
      queryClient.getQueryState(queryKeys.docs("work-items"))?.status,
    ).toBe("success");
  });
