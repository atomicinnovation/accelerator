...
import type { IndexEntry, LifecycleCluster } from "../../api/types";
...
describe("LifecycleClusterContent", () => {
  ...
  it("renders a Pipeline panel above the timeline with cluster completeness", async () => {
    ...
    expect(
      panel.querySelector(".ac-lcluster__pipeline-eyebrow")!.textContent,
    ).toMatch(/^Pipeline$/i);
    ...
    expect(
      panel.querySelector("ol.ac-stagechain")!.getAttribute("data-variant"),
    ).toBe("panel");
  });
  ...
  it("renders a stage tile for each timeline step", async () => {
    ...
    const timelineSteps = container.querySelectorAll(
      "ol:not(.ac-stagechain) > li[data-stage]",
    );
    expect(timelineSteps.length).toBeGreaterThan(0);
    // Each step's tile carries data-active reflecting whether the stage
    // has a present entry.
    const planTile = timelineSteps[0].querySelector(
      "[data-active][data-stage]",
    )!;
  ...
  it('shows a "no such cluster" message and the eyebrow back-link on 404', async () => {
    ...
    expect(
      screen.getByRole("link", { name: /^Lifecycle$/i }),
    ).toBeInTheDocument();
  });
  ...
  it("appends design inventory and design gap entries to the timeline", async () => {
    const withDesignDocs: LifecycleCluster = {
      ...
      entries: [
        ...
        entry(
          "design-inventories",
          "meta/design-inventories/2026-05-01-foo.md",
          "Foo Design Inventory",
          150,
        ),
        entry(
          "design-gaps",
          "meta/design-gaps/2026-05-02-foo.md",
          "Foo Design Gap",
          160,
        ),
      ],
    ...
    expect(
      screen.queryByText(/no design inventory yet/i),
  ...
  it("renders an entry status frontmatter as a Chip with the right variant", async () => {
    ...
    expect(
      container.querySelector('span[data-variant="green"]'),
    ).not.toBeNull();
  });
  ...
  it("renders a neutral chip for unknown status", async () => {
    ...
    await screen.findByText("P");
    expect(
      container.querySelector('span[data-variant="neutral"]'),
    ).not.toBeNull();
  });
  ...
  it("renders the cluster status chip in the header subtitle from the work-item entry", async () => {
    ...
    const headerSubRow = container.querySelector(
      `.${(/_subRow_[^\s"]+/.exec(container.innerHTML) ?? [""])[0]}`,
    );
  ...
  it("renders multiple entries within a single stage", async () => {
    const multi: LifecycleCluster = {
      ...
      entries: [
        ...
        entry(
          "plan-reviews",
          "meta/reviews/plans/foo-review-1.md",
          "Foo plan review 1",
          110,
        ),
        entry(
          "plan-reviews",
          "meta/reviews/plans/foo-review-2.md",
          "Foo plan review 2",
          130,
        ),
      ],
