...
import { FetchError, fetchLifecycleClusters } from "../../api/fetch";
...
function statusOf(cluster: LifecycleCluster): string | null {
  ...
  if (entry?.frontmatterState !== "parsed") return null;
...
function SortSegment({ value, onChange }: SortSegmentProps) {
  return (
    // biome-ignore lint/a11y/useSemanticElements: intentional div+role="group" to preserve the segmented-control layout/styling; a native <fieldset> brings default chrome and box model that would change rendering
