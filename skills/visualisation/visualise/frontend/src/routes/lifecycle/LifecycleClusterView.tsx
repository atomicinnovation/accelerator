...
import { FetchError, fetchLifecycleCluster } from "../../api/fetch";
...
function statusOf(cluster: LifecycleCluster): string | null {
  ...
  if (wi?.frontmatterState !== "parsed") return null;
...
export function LifecycleClusterContent({ slug }: { slug: string }) {
  ...
  const eyebrow = (
    <>
      ...
      <Link to="/lifecycle" className={styles.eyebrowLink}>
        Lifecycle
      </Link>
    </>
  );
  ...
  return (
    <Page eyebrow={eyebrow} title={cluster.title} subtitle={subtitle}>
      <section className={`${styles.pipelinePanel} ac-lcluster__pipeline`}>
        <div
          className={`${styles.pipelineEyebrow} ac-lcluster__pipeline-eyebrow`}
        >
          Pipeline
        </div>
      ...
      <ol className={styles.timeline}>
        {buildTimeline(cluster).map((slot) =>
          slot.entry ? (
            <TimelineStep
...
function TimelineStep({ step, entry, cluster }: TimelineStepProps): ReactNode {
  ...
  const fmStatus =
    entry.frontmatterState === "parsed" ? entry.frontmatter.status : undefined;
  const fmVerdict =
    entry.frontmatterState === "parsed" ? entry.frontmatter.verdict : undefined;
  const fmDate =
    entry.frontmatterState === "parsed" ? entry.frontmatter.date : undefined;
      ...
      >
        <div className={styles.tcardHead}>
          <div className={styles.tcardHeadLeft}>
            <span className={styles.tcardStage}>
          ...
          <div className={styles.tcardHeadRight}>
            {typeof fmStatus === "string" && fmStatus.length > 0 && (
              <Chip variant={statusToVariant(fmStatus)} size="sm">
                {fmStatus}
              </Chip>
            )}
            {typeof fmVerdict === "string" && fmVerdict.length > 0 && (
              <Chip variant={statusToVariant(fmVerdict)} size="sm">
                {fmVerdict}
              </Chip>
            )}
...
function MissingStep({ step }: { step: Step }): ReactNode {
  return (
    <li
      className={`${styles.step} ${styles.stepMissing}`}
      data-stage={step.docType}
    >
      <span className={styles.stepNode}>
