...
import { resolveToken, parseRgb, setTheme } from './lib/expected-colours'
...
function decomposeMatrix(transform: string): { rotationDeg: number; scale: number } {
...
for (const theme of ['light', 'dark'] as const) {
  test.describe(`kanban-card-resolved-styles (${theme})`, () => {
    ...
    test('dragging card: rotate(1.5deg) scale(1.02), lift shadow, accent border', async ({ page }) => {
      const el = page.locator(card('dragging'))
      const { transform, boxShadow, borderColor } = await el.evaluate((node) => {
