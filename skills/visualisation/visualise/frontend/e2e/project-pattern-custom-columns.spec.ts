...
const PROJ_WORK_ITEMS = [
  makeWorkItem(
    "meta/work/PROJ-0001-first-story.md",
    "first-story",
    "PROJ-0001",
    "First Story",
    "ready",
  ...
  makeWorkItem(
    "meta/work/PROJ-0002-second-story.md",
    "second-story",
    "PROJ-0002",
    "Second Story",
    "in-progress",
  ...
  makeWorkItem(
    "meta/work/PROJ-0007-seventh-story.md",
    "seventh-story",
    "PROJ-0007",
    "Seventh Story",
    "done",
...
test.describe("Scenario B — project ID pattern + custom columns", () => {
  test("renders only the four configured columns", async ({ page }) => {
    ...
    for (const key of ["draft", "blocked", "abandoned"]) {
      await expect(
        page.locator(`section[data-column="${key}"]`),
      ).not.toBeVisible();
    }
  });

  test("PROJ-prefixed work-item cards render in the correct columns", async ({
    page,
  }) => {
    await mockKanbanConfig(page);
    ...
    await expect(
      page.locator('section[data-column="ready"]').getByText("First Story"),
    ).toBeVisible();
    await expect(
      page
        .locator('section[data-column="in-progress"]')
        .getByText("Second Story"),
    ).toBeVisible();
    await expect(
      page.locator('section[data-column="done"]').getByText("Seventh Story"),
    ).toBeVisible();
  });

  test("PATCH with status outside the seven defaults returns 400", async ({
    request,
  }) => {
  ...
  test("PATCH with label-cased status returns 400 (case sensitive)", async ({
    request,
  }) => {
    const docsRes = await request.get("/api/docs?type=work-items");
    const { docs } = (await docsRes.json()) as {
