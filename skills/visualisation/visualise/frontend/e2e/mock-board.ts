 ...
 */
export async function installMockBoard(page: Page, opts: MockBoardOptions): Promise<void> {
  ...
  await page.route('**/api/docs/**/frontmatter', async (route) => {
    ...
    const encoded = pathname.replace(/^.*\/api\/docs\//, '').replace(/\/frontmatter$/, '')
    ...
    await route.fulfill({ status: 200, headers: { etag: `"e2e-mock-${tick}"` }, body: '' })
  })
}
