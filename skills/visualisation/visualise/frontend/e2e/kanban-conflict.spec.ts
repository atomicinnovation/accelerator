...
test("stale ETag produces an assertive error toast and reverts the card to draft", async ({
  page,
}) => {
  ...
  await page.route("**/api/docs/**/frontmatter", async (route) => {
    ...
    writeFileSync(
      WORK_ITEM_PATH,
      original.replace("status: draft", "status: done"),
    );
  ...
  try {
    ...
    await expect(
      errorRegion.getByText(/updated by another editor/i),
    ).toBeVisible();
