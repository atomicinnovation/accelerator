...
test("resolved wiki-link [[ADR-0001]] renders as a clickable anchor", async ({
  page,
}) => {
...
test("resolved wiki-link [[WORK-ITEM-0001]] renders as a clickable anchor", async ({
  page,
}) => {
  await page.goto("/library/plans/first-plan");
  await expect(page.locator("article")).toBeVisible();
...
test("unresolved wiki-link [[ADR-9999]] renders as a span, not an anchor", async ({
  page,
}) => {
  await page.goto("/library/plans/first-plan");
  await expect(page.locator("article")).toBeVisible();
