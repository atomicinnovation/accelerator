...
import { diffImages, pixelDeltaE, decodePng, main } from './visual-diff-ciede2000'
import { writeFileSync, mkdtempSync } from 'node:fs'
import { tmpdir } from 'node:os'
import { join } from 'node:path'

function makePng(width: number, height: number, fill: [number, number, number, number]): PNG {
