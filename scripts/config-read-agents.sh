...
# Thin shim: route to the `a9r config-read-agents` subcommand when a trusted
...
#
# The canonical AGENT_KEYS list lives in config-read-agents-impl.sh (also
# mirrored by a9r-core's agents module and asserted against agents/*.md).
...
if [ -z "${A9R_FORCE_BASH:-}" ] && bin="$(a9r_bin 2>/dev/null)" && [ -n "$bin" ]; then
  exec "$bin" config-read-agents "$@"
fi
exec "$SCRIPT_DIR/config-read-agents-impl.sh" "$@"
