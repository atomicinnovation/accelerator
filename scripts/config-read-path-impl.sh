...
# Bash implementation of config-read-path. Invoked by the config-read-path.sh
# shim when the `a9r` binary is unavailable or A9R_FORCE_BASH is set. Kept
# byte-for-byte identical to the pre-shim script (including its user-facing
# "config-read-path.sh" usage/warning strings) so the bash fallback and the
# a9r path remain indistinguishable to callers and the parity gate.
#
