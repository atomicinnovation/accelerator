...
# Bash implementation of config-read-value. Invoked by the config-read-value.sh
...
# "config-read-value.sh" usage string) so the bash fallback and the a9r path
# remain indistinguishable to callers and the parity gate.
#
