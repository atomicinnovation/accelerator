...
- A default remote set for the repo (`gh repo set-default`) so PR
  numbers resolve.
- `github.token` (or `github.token_cmd`) configured, or `GH_TOKEN`/
  `GITHUB_TOKEN` set — see [the collaboration
  docs](../collaboration.md) — for the parts of these workflows that
  call the GitHub API directly rather than through `gh`.
