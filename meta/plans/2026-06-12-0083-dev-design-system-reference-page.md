...
> darwin). The only outstanding items (the unticked boxes below) are **operational/
> manual handoffs, not implementation**: (1) the **linux VR baselines** — regenerated
> by the user via the "Update visual regression baselines" CI workflow (darwin
> baselines are regenerated + committed; a `GITHUB_TOKEN` push won't re-trigger Main
> CI, so linux VR is expected red until that workflow runs); (2) the **manual
> browser eyeball** verifications per phase; and (3) the **cross-browser chord
> matrix** (the dedicated manual AC — the chord stayed `Cmd/Ctrl+Shift+L`; no
> fallback was forced). Commits: Icon primitive → icon migration → AtomicMark/Glyph →
> /dev route+activation+chrome → scroll-spy+theme → tokens/type → glyph/mark/icon →
> interactive primitives → composites & chrome → VR migration + scroll-spy hardening
> → retire showcases + README + AC reconciliation. Notable deviations are recorded
> inline per phase (Toaster ToastIcon kept
...
- [x] `mise run frontend:check` passes (renamed specs + new e2e type-check + lint)
- [x] `mise run test:e2e:visualiser` passes with the renamed specs + regenerated
  baselines (all 9 migrated specs assert `DevDesignSystem` sections — **301 darwin
  cases green** incl. the repointed `typography-resolved-sizes` kanban case + the
  4 deferred Phase 5/6 behavioural e2e). *Darwin baselines regenerated locally;
  **linux baselines are the user's CI handoff** via the "Update visual regression
  baselines" workflow (a `GITHUB_TOKEN` push won't re-trigger Main CI), so linux VR
  is expected red until that workflow runs.*
- [x] **Automated cell-presence gate**: the `VR cell-presence gate (Phase 10)`
  describe in `DevDesignSystem.test.tsx` enumerates every required `data-testid`
  with **pinned dimensions** — `glyph-cell-<type>-{16,24,32,48}`,
  `big-glyph-cell-<type>`, `chip-cell-<variant>-{sm,md}`, `code-syntax-cell-<lang>`
  for all 8 + bash, `kanban-card-cell-<state>` — and fails if any is missing
- [x] No orphaned snapshot folders (old `*-showcase.spec.ts-snapshots/` renamed to
  `dev-design-system-*-snapshots/`; stale full-page code-syntax PNGs removed)
- [x] Code-syntax spec is a clipped per-`code-syntax-cell-<lang>` locator (no
...
- [ ] linux + darwin baselines committed for every renamed pixel case *(darwin
  committed; **linux pending the "Update visual regression baselines" workflow**)*

> **Phase 10 notes/deviations.** (1) The five pixel specs + four resolved specs are
> renamed `dev-design-system-*` and repointed at `/dev#<section>` (glyph/glyph-fill
> →`#glyphs`, big-glyph→`#bigglyphs`, chip/chip-colours→`#chips`,
> code-syntax/code-colours→`#code`, kanban/kanban-styles→`#cards`); snapshot folders
> renamed alongside. `playwright.glyph.config.ts` testMatch + `/dev` health route
> updated. The non-migrated `typography-resolved-sizes` kanban-kind-badge case was
> also repointed `/kanban-card-showcase`→`/dev#cards` (same `KANBAN_ENTRY` kind) so
> Phase 11's route deletion won't break it. (2) **All darwin baselines regenerated**
> (the `/dev` cell rendering differs from the retired showcases) — incl. the net-new
> glyph-48 cells and the per-language code-syntax clips. **Linux is the user's CI
> handoff.** (3) **The deferred Phase 5/6 e2e** (`e2e/dev-design-system.spec.ts`)
> now passes: deep-link landing + active TOC, scroll-spy advance + never-pinned,
> scroll-to-top clears hash, and the colours theme-responsiveness check (a surface
> swatch's computed `backgroundColor` resolves to the dark `--ac-bg` token after the
> in-page toggle). (4) **Two scroll-spy hardenings surfaced by the e2e** (committed
> with this phase): the deep-link landing scroll is deferred to `fonts.ready` + rAF
> so it lands at the settled offset; the spy now also recomputes on an rAF-coalesced
> `scroll` event (the IntersectionObserver fires on threshold crossings, not on
> settle); and `pickActiveSection` is called with a 2px band slop to absorb
> sub-pixel `getBoundingClientRect` rounding (a deep-linked section lands at e.g.
> 80.3px against an exact 80px band edge). The pure `pickActiveSection` helper +
...
- [x] `mise run frontend:check` and `mise run test:unit:frontend` pass (2445 unit
  tests; 117 files — the 4 showcase test files removed, +1 router not-found test)
- [x] Router test: each former showcase path (`/glyph-showcase`, etc.) no longer
  resolves to a showcase component (default not-found, no redirect) — asserts the
  pathname is unchanged and no match carries the retired path as a route id
- [x] `mise run test:e2e:visualiser` passes (specs target `/dev`, not the removed
  routes) — darwin green; linux baselines via the user's CI workflow
- [x] No dangling imports of the deleted route components (build clean — `tsc -b`
  + `vite build` succeed; the `KanbanCardShowcase.module.css` migration exception
  was removed and `AC5_FLOOR` lowered 1622→1546 for the deleted tokenised CSS)

#### Manual Verification

- [x] README developer-routes section lists `DevDesignSystem` and none of the
  five showcases, and documents all three triggers, the chord, the canonical
  `/dev#<section>` form, and the slug-vs-label distinction (the cross-browser
  matrix did not force the `Cmd/Ctrl+Shift+G` fallback, so the chord stays
  `Cmd/Ctrl+Shift+L`)
- [x] Work item 0083 ACs amended (AC3/AC5/AC7 were already in the route+bare-section
  form; AC9 → "SPA not-found, HTTP 200, no redirect"; stale count parentheticals
  reconciled to Stage dots 8 / Doc-type glyphs 13, sizes 16/24/32/48)
- [ ] Visiting a former showcase path shows the not-found UI, not a stale page
  (manual browser check)

> **Phase 11 notes.** The five showcase routes + their `src/routes/*-showcase/`
> dirs (components, tests, module CSS) are deleted; `router.ts` imports + routeTree
> entries removed. Stale `/glyph-showcase` / `/big-glyph-showcase` references in the
> `Glyph`/`BigGlyph`/`types.ts` comments were repointed at `/dev#glyphs` /
> `/dev#bigglyphs`. The `use-dev-activation` "stored prior no longer resolves" test
> already used `/glyph-showcase` as its non-resolving sample — now genuinely gone,
> so it still falls back to `/library`.
