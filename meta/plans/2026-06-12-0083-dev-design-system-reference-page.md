...
> **Implementation progress (2026-06-13).** Phases **1–10 are implemented and
> committed**, each leaving `main` green on darwin (`mise run frontend:check` + 2488
> unit tests; the 9 migrated VR specs + the deferred behavioural e2e — 301 darwin
> cases). Per-phase success-criteria checkboxes below are ticked accordingly; the
> unticked boxes in phases 1–10 are the remaining **manual/browser** verifications
> and **the linux VR baselines** (regenerated by the user via the "Update visual
> regression baselines" CI workflow — darwin baselines are regenerated + committed).
> Only **Phase 11 remains** (retire the 5 showcase routes + components, rewrite the
> README, amend work-item 0083 ACs). Commits: Icon primitive → icon migration →
> AtomicMark/Glyph → /dev route+activation+chrome → scroll-spy+theme → tokens/type →
> glyph/mark/icon → interactive primitives → composites & chrome → VR migration +
> scroll-spy hardening. Notable deviations are recorded inline per phase (Toaster
> ToastIcon kept
