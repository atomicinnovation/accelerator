...
id: "2026-06-06-0086-kanban-convergence-record"
title: "Kanban Drag-and-Drop Convergence Record (work item 0086)"
schema_version: 1
last_updated: "2026-06-06T22:35:00+00:00"
...
| 9 | Typography scale | **fixed** | The card title and column heading inherited the 16px UA base (no explicit size) and the id/slug/foot/badge/empty-body used tokens a step larger than the prototype. Re-pinned to the prototype's compact scale via existing tokens — column title + card title `--size-subtitle` (13), id/slug/foot `--size-3xs-lg` (10.5), kind badge `--size-4xs` (9.5), empty title `--size-xs` (14), empty body `--size-row` (12.5). Locked by four new cases in `typography-resolved-sizes.spec.ts`. The app-wide 16px body base vs the prototype's 14px is a separate, documented matter owned by ADR-0036 / spike 0091 — left unchanged here as it does not affect the kanban surfaces (every kanban text element now sets an explicit token). |
