...
**Path**: `<paths.design_inventories>/YYYY-MM-DD-{source}/inventory.md` (default `meta/research/design-inventories/`)
...
**Path**: `<paths.design_gaps>/YYYY-MM-DD-{slug}.md` (default `meta/research/design-gaps/`)
...
type: design-gap
current_inventory: meta/research/design-inventories/2026-05-02-current-app/inventory.md
target_inventory: meta/research/design-inventories/2026-05-02-prototype/inventory.md
author: Toby Clemson
status: draft                      # draft | accepted | superseded
...
**Required**: add `meta/research/design-inventories/` and `meta/research/design-gaps/` to the init list, update the count to 14, update the summary block.
...
3. **Diff-friendly markdown.** The `inventory.md` files diff cleanly across snapshots (`jj diff meta/research/design-inventories/2026-05-02-current-app/inventory.md meta/research/design-inventories/2026-06-15-current-app/inventory.md`) without screenshot binaries entering the diff stream unless explicitly requested.
...
- **Pro**: directory-per-inventory diverges from the flat-file convention used by `meta/research/codebase/`, `meta/plans/`, etc., but the divergence is justified by the unique asset-bearing requirement of inventories.
...
#### 9.8 Nested layout under `meta/research/codebase/`

Considered. The user raised the option of restructuring `meta/research/codebase/` into subdirectories (e.g. `research/codebase/`, `research/ideas/`, `research/design-inventories/`). This is a reasonable structural improvement but is **a separate refactor**: it changes how every existing research file is organised, affects the `research-codebase` skill's path resolution, and warrants its own ADR. Bundling it with this work would conflate two changes. The flat layout (`meta/research/design-inventories/`, `meta/research/design-gaps/`) is recommended for now; the nested-research restructure is noted as a future follow-up (see `meta/notes/2026-05-02-research-directory-subcategory-restructure.md`).
...
- The existing `meta/research/codebase/` body shows the prose conventions the gap artifact mirrors: paragraph-rich, action-oriented language is already the norm for research documents, which is partly why `extract-work-items` operates on natural-language patterns rather than structural markers.
...
No prior research documents in `meta/research/codebase/` directly address frontend convergence, design inventory, or browser automation in this plugin. This document establishes the problem space.
...
6. **Future research/ subcategory restructure.** The user raised the idea of nesting `meta/research/codebase/` into subdirectories (`research/codebase/`, `research/ideas/`, etc.). Worth its own ADR. The introduction of `design-inventories` could be the forcing function — but as a separate, clearly-scoped change.
