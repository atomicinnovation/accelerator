...
topic: "Restructure meta/research/ into subject subcategories (work item 0056)"
...
# Research: Restructure `meta/research/` into Subject Subcategories — Work Item 0056
...
6. The visualiser prebuilt JS bundle (`skills/visualisation/visualise/frontend/dist/assets/index-DSMqnoSb.js`)
   ...
   not rewrite by hand.
7. **A stray `.DS_Store` under `meta/design-inventories/`** that should be
...
**`meta/research/` — flat, single-file shape, 37 `*.md` + `.gitkeep` (38 entries)**.
...
`meta/research/2026-05-08-0052-documents-locator-config-driven-paths.md`).

**`meta/design-inventories/` — directory-per-investigation, 2 inventories**:
- `meta/design-inventories/.gitkeep`
- `meta/design-inventories/.DS_Store` — **stray macOS metadata, FLAG: exclude from move**
...
**`meta/design-gaps/` — flat, single-file, 1 `*.md` + `.gitkeep`**:
- `meta/design-gaps/.gitkeep`
...
- `codebase/` — flat single-file ✓ (all 37 existing `meta/research/*.md`)
...
|---|---|---|
| `meta/research/` literal path | scripts: 3 files; skills: ~15 files | README: 4 lines; CHANGELOG: 1 line; meta/: 250+ lines across ~100 files |
| `meta/design-inventories/` literal | scripts: 6; skills: ~14 (incl. Rust server + Vite frontend) | README: 1 line; meta/: ~50 lines across ~20 files |
| `meta/design-gaps/` literal | scripts: 6; skills: ~12 | README: 5 lines; CHANGELOG: 3 lines; meta/: ~50 lines across ~25 files |
...
- `README.md:42` — `meta/research/      meta/plans/     checked-off plan` (ASCII diagram)
- `README.md:47` — `research document in \`meta/research/\` with findings,`
- `README.md:64` — `` `meta/research/` ``
- `README.md:84-95` — `meta/` table (rows for `research/`, `design-inventories/`, `design-gaps/`)
- `README.md:409` — `meta/research/    meta/plans/` (ASCII diagram, ADR section)
- `README.md:413` — `meta/decisions/` (same diagram, anchors `meta/research/` flow)
- `README.md:579` — `` The resulting gap artifact under `meta/design-gaps/` `` **(sixth occurrence — not flagged in work item line 240)**

**CHANGELOG.md mentions** (will likely need synchronisation in the same
commit):
- `CHANGELOG.md:98` — `produces an RCA document in \`meta/research/\``
...
for renames:
- index 1: `"meta/research"` → `"meta/research/codebase"`
- index 13: `"meta/design-inventories"` → `"meta/research/design-inventories"`
- index 14: `"meta/design-gaps"` → `"meta/research/design-gaps"`

**One new key appended**: `paths.research_issues` → `meta/research/issues`.
...
discover the parent `meta/research/` directory?
...
```
Line 42 (`meta/research/      meta/plans/     checked-off plan`) needs
...
```
Line 409 (`meta/research/`) → `meta/research/codebase/`.
...
location: `research document in `meta/research/` with findings,…`

**`README.md:64`** — narrative description of `research-issue` output
location: `produces an RCA document in `meta/research/`.`
...
7. **Stray `.DS_Store`** under `meta/design-inventories/`. Exclude from move
...
of `meta/research/foo.md` and the appearance of `meta/research/codebase/foo.md`
...
- **`meta/notes/2026-05-02-research-directory-subcategory-restructure.md`** —
  the source note. Frames `meta/research/` as a flat directory mixing
  ...
  table updates.
- **`meta/research/2026-05-02-design-convergence-workflow.md`** — the
  research artifact whose §9.8 ("Nested layout under `meta/research/`")
...
- `meta/research/2026-05-02-design-convergence-workflow.md` — design-convergence
  workflow that introduced `meta/design-inventories/` and `meta/design-gaps/`
