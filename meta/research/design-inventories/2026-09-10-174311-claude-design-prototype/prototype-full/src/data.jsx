...
const DOC_TYPES = [
  ...
  { key: "topic-research",      label: "Topic research",     count: 4,  pulse: true,  kind: "doc" },
  { key: "research",            label: "Codebase research",  count: 12, pulse: false, kind: "doc" },
...
const LIBRARY_GROUPS = [
  ...
  { key: "discover", label: "Discover", types: ["design-inventories", "design-gaps", "topic-research", "research"] },
...
const LIBRARY_INDEX = {
  ...
  "topic-research": [
    { id: "model-context-protocol", title: "Model Context Protocol",        date: "2026-05-04", status: "synthesised", slug: "model-context-protocol" },
    { id: "agentic-eval-harnesses", title: "Agentic evaluation harnesses",  date: "2026-05-02", status: "gathering",   slug: "agentic-eval-harnesses" },
    { id: "prompt-caching-economics", title: "Prompt caching economics",    date: "2026-04-28", status: "outlined",    slug: "prompt-caching-economics" },
    { id: "sse-vs-websocket-transports", title: "SSE vs WebSocket transports", date: "2026-04-21", status: "briefed",  slug: "sse-vs-websocket-transports" },
  ],
  research: [
...
`,
    ...
    case "topic-research": {
      // A topic-research doc is the *manifest* of a research set: one subject,
      // many documents accreted under meta/research/topics/<slug>/. The shape
      // below mirrors what the research engine writes to disk.
      const status = row.status || "briefed";
      const rounds = { briefed: 0, outlined: 1, gathering: 2, synthesised: 2 }[status] ?? 0;
      const FINDINGS = {
        "model-context-protocol": [
          "what-is-mcp", "transport-and-framing", "capability-negotiation",
          "server-ecosystem", "client-support", "security-model",
          "versioning-and-stability",
        ],
        "agentic-eval-harnesses": [
          "harness-landscape", "task-suite-design", "grading-strategies",
          "cost-and-runtime-profile", "regression-tracking",
        ],
      };
      const findings = status === "briefed" || status === "outlined" ? [] :
        (FINDINGS[slug] || ["landscape", "prior-art", "trade-offs", "open-questions"]);
      const TAGS = {
        "model-context-protocol": ["research", "protocols"],
        "agentic-eval-harnesses": ["research", "evaluation"],
        "prompt-caching-economics": ["research", "cost"],
        "sse-vs-websocket-transports": ["research", "transports"],
      };
      const stamp = `${date}T14:02:00+00:00`;
      const updated = `${date}T15:20:00+00:00`;
      const subjectLine = {
        "model-context-protocol": "Research into the Model Context Protocol (MCP) — its wire format, transport\nlayer, capability negotiation, and the ecosystem of servers and clients — to\nground a decision on whether to expose Accelerator's tools over it.",
        "agentic-eval-harnesses": "Research into agentic evaluation harnesses — task-suite design, grading\nstrategies, and cost profiles — to ground a decision on how Accelerator's\nreview agents should be measured between releases.",
        "prompt-caching-economics": "Research into prompt caching economics across providers — cache lifetimes,\nbilling models, and hit-rate behaviour under agent fan-out — to ground a\ndecision on how the orchestrator should structure its prompts.",
        "sse-vs-websocket-transports": "Research into server-sent events versus WebSocket transports — reconnect\nsemantics, proxy behaviour, and browser limits — to ground a decision on the\nvisualiser's live-update channel.",
      }[slug] || `Research into ${title} — the landscape, the prior art, and the trade-offs —\nto ground a downstream decision.`;
      return {
        type, slug,
        frontmatter: {
          type: "topic-research",
          ...
          primary: status === "synthesised" ? "synthesis.md" : status === "gathering" ? "outline.md" : "brief.md",
          tags: TAGS[slug] || ["research"],
...
- Brief: \`brief.md\`
- Outline: \`outline.md\`${rounds ? ` (${rounds} round${rounds === 1 ? "" : "s"})` : " (not yet drafted)"}
- Findings (${findings.length}):
${findings.length === 0 ? "  - none yet" : findings.map((f, n) => `  - \`findings/${String(n + 1).padStart(2, "0")}-${f}.md\``).join("\n")}
- Synthesis: ${status === "synthesised" ? "\`synthesis.md\`" : "not yet written"}
- Reports: none yet
`,
      };
    }
