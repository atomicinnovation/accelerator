...
function Icon({ name, size = 18, className = "", style }) {
  ...
  const paths = {
    ...
    moon: <><path d="M20 14a8 8 0 0 1-10-10 8 8 0 1 0 10 10" style={{ strokeWidth: "2px" }} /></>,
...
function StatusBadge({ status }) {
  const map = {
    ...
    // Topic-research lifecycle: brief → outline → findings → synthesis.
    "briefed": { tone: "neutral", label: "Briefed" },
    "outlined": { tone: "indigo", label: "Outlined" },
    "gathering": { tone: "amber", label: "Gathering" },
    "synthesised": { tone: "green", label: "Synthesised" },
...
function renderMarkdown(src) {
  ...
  while (i < lines.length) {
    ...
    if (/^[-*]\s/.test(l)) {
      // Indent-aware: two spaces per level, so manifests can nest file lists.
      const raw = [];
      while (i < lines.length && /^\s*[-*]\s/.test(lines[i])) {
        const m = lines[i].match(/^(\s*)[-*]\s(.*)$/);
        raw.push({ depth: Math.floor(m[1].length / 2), text: m[2] });i++;
      }
      const items = raw.map((r) => r.text);
      ...
      const buildList = (idx, depth) => {
        const kids = [];
        while (idx < raw.length && raw[idx].depth >= depth) {
          if (raw[idx].depth > depth) {
            const [sub, ni] = buildList(idx, raw[idx].depth);
            const prev = kids.pop();
            kids.push(<li key={"n" + idx}>{prev ? prev.props.children : null}{sub}</li>);
            idx = ni;
            continue;
          }
          kids.push(<li key={idx}>{inline(raw[idx].text)}</li>);
          idx++;
        }
        return [<ul key={"ul" + depth + "-" + idx} className="ac-md-ul">{kids}</ul>, idx];
      };
      out.push(<React.Fragment key={"ul" + i}>{buildList(0, 0)[0]}</React.Fragment>);
      continue;
    }
...
function renderEmphasis(t, key) {
  ...
  while (rest.length) {
    ...
    const i = rest.match(/\*([^*]+)\*/);
    let pick = null;
    if (b && (!i || b.index <= i.index)) pick = { m: b, tag: "strong" };else
    if (i) pick = { m: i, tag: "em" };
...
const TYPE_META = {
  ...
  research: { hue: 28, label: "Codebase research", short: "RSC" },
  // Topic research — a research *set* per external subject. Green sits clear of
  // research (28) and the design-* types (185 / 95); `l` darkens the foreground
  // so the tinted glyph clears 3:1 against the page background.
  "topic-research": { hue: 132, l: 34, label: "Topic research", short: "TRS" },
...
const TYPE_ICONS = {
  ...
  // A research *set*: layered sheets under one dossier cover, with separate
  // strands converging into a single node — distinct from `research`, which is
  // one write-up under a magnifier.
  "topic-research":
  <g fill="none" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round">
      ...
      <path d="M10 9h7.5" />
      <path d="M10 12.5 14 15.5M10 15.5h3.6M10 18.5 14 15.5" strokeWidth="1.15" />
...
function TypeGlyph({ type, size = 28 }) {
  ...
  return (
    <div className="ac-glyph" style={{
      width: size, height: size,
      color: `hsl(${meta.hue} 68% ${meta.l != null ? meta.l : 44}%)`,
