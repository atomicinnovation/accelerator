...
- [ ] Given Glyph fill colours are defined via `--ac-doc-<key>` tokens, when `document.documentElement[data-theme]` is toggled between `light` and `dark`, then after one `requestAnimationFrame` yield, `getComputedStyle(glyphSvg).fill` resolves to the corresponding dark-mode hex from the Colour Token Table, and no React state change has occurred (asserted via a render counter on the Glyph component).
...
- [ ] **Playwright visual-regression snapshot**. A spec under `e2e/glyph.spec.ts` renders the `/glyph-showcase` route at viewport `1024×768`, captures a screenshot per theme, and compares against committed baselines at `e2e/__snapshots__/glyph-showcase-{light,dark}.png` derived from `library-view-updated-{light,dark}.png` cropped to the glyph grid. The spec passes when pixel-diff ≤ 0.5 % per snapshot, configured via Playwright's `toHaveScreenshot({ maxDiffPixelRatio: 0.005 })`.
...
- How should icon assets be packaged — per-doc-type SVG React components imported individually, a single sprite sheet referenced by `<use href="#…">`, or an inline-SVG map keyed by `DocTypeKey`? The choice affects bundle size and tree-shaking but does not block the Glyph public API; flagging for resolution during implementation rather than now.
