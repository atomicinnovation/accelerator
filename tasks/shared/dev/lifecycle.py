...
class DevDeps:
    ...
    Default-constructed by the ``@task`` adapters to the real circus /
    subprocess / psutil / time wiring; the unit tests build it with fakes.
    Bundling the paths here means ``bring_up`` never reaches into
    ``tasks/dev.py`` constants, so the render-then-launch ordering and
    directory side-effects live in one place.
    """
...
class _UpAbortError(Exception):
    def __init__(self, result: UpResult) -> None:
        self.result = result
...
def log_diagnostic(deps: DevDeps, message: str) -> None:
    ...
    with contextlib.suppress(OSError):
        ...
        with Path(deps.diagnostic_log).open("a") as handle:
            handle.write(line + "\n")
...
def _read_server_info(path: Path | str) -> dict[str, Any] | None:
    try:
...
def _probe_status(
    state: DevState, deps: DevDeps, *, timeout: float
) -> dict[str, str] | None:
...
def _arbiter_identity_ok(state: DevState, deps: DevDeps) -> bool:
    if state.arbiter_pid is None or state.arbiter_start_time is None:
        return False
    return deps.killer.is_alive(
        state.arbiter_pid
    ) and deps.killer.identity_matches(
...
def reuse_or_teardown(state: DevState | None, deps: DevDeps) -> ReuseOutcome:
    ...
    Because the endpoint is a per-workspace ``ipc://`` socket, a reachable
    probe can only be *this* workspace's arbiter; the recorded identity is
    confirmed as belt-and-braces before reuse. A degraded/stale session is
    torn down; if teardown can't confirm death (survivor/refused) the caller
    fails fast rather than launching a competitor.
    """
    if state is None:
...
def bring_up(deps: DevDeps) -> UpResult:
    """Start (or reuse) the supervised dev stack.

    The orchestrator entry point.
    """
    with workspace_lock(deps.lock_path) as locked:
        if not locked:
            # Another `dev` is mid-launch in this workspace: re-probe and
            # reuse if it is now healthy, else fail fast (never race into a
            # duplicate).
            state = read_dev_state(deps.state_path)
            if state is not None:
                statuses = _probe_status(
                    state, deps, timeout=deps.probe_timeout
                )
                if (
                    statuses is not None
                    and evaluate_health(statuses) == Health.HEALTHY
            ...
            return UpResult(
                ...
                message=(
                    ...
                    "moment, or run `mise run dev:status` / "
                    "`mise run dev:stop` if this persists"
                ),
            )
...
def _bring_up_locked(deps: DevDeps) -> UpResult:
    ...
    if outcome.action == "reuse" and outcome.state is not None:
        return _reused_result(outcome.state, deps)
    if outcome.action == "abort" and outcome.stop_result is not None:
    ...
    except _UpAbortError as abort:
        return abort.result
    return _started_result(launched, deps)


def allocate_and_launch(deps: DevDeps) -> LaunchedArbiter:
    """Allocate ports/sockets, write state, then launch and confirm circusd."""
    ...
    write_dev_state(
        deps.state_path, state
    )  # provisional: PIDs null, discoverable
    ...
    if pid is None:
        # ipc:// endpoints have no port to collide on, so a
        # non-appearing/empty pidfile is a genuine failure (bad INI, missing
        # binary), not a transient race: reap the launch handle (the real
        # arbiter) and fail without retry.
        _reap_handle(handle, deps)
        log_diagnostic(
            deps, f"circusd did not write a valid pidfile at {deps.pidfile}"
        )
        raise _UpAbortError(
            UpResult(
                "failed",
    ...
    if start is None or not deps.killer.is_alive(pid):
        ...
        raise _UpAbortError(
            UpResult(
                "failed",
                message=(
                    f"the circus daemon exited immediately; see "
                    f"{deps.dev_dir}/circusd.log"
                ),
                artifact=f"{deps.dev_dir}/circusd.log",
            )
        )
...
def _poll_arbiter_pid(deps: DevDeps) -> int | None:
    """Bounded poll for a valid integer pidfile; empty/partial keeps polling."""
...
def _readiness_gate(launched: LaunchedArbiter, deps: DevDeps) -> None:
    if not wait_for_file(
        ...
        log_diagnostic(
            deps, "server-info.json did not appear before the readiness timeout"
        )
        raise _UpAbortError(
            UpResult(
                "failed",
                message=(
                    f"server did not write {deps.server_info_path} in time; "
                    f"see {deps.dev_dir}/server.log"
                ),
        ...
        try:
            ...
            raise _UpAbortError(
                UpResult(
                    "failed",
                    message=(
                        f"could not start the frontend watcher: {exc}; see "
...
def _record_watcher_pid(
    launched: LaunchedArbiter, deps: DevDeps, name: str
) -> None:
    """Record a watcher PID + start-time into dev-state once it is active.

    Incremental (not batched) so there is no launch-window gap where a watcher
    is live but unrecorded — teardown can then reach reparented orphans by
    identity even if the arbiter dies.
    """
    sup = deps.client_factory(launched.endpoint, timeout=deps.probe_timeout)
    try:
        pids: list[int] = sup.pids(name)
    except SupervisorUnreachableError:
        pids = []
    finally:
        _maybe_close(sup)
...
class TargetSet:
    ...
    descendants: list[
        tuple[int, float]
    ]  # pre-quit snapshot (recycled-PID baseline)
    # recorded (pid, start_time) for server/frontend — the orphan-reach
    # baseline, gated against the *recorded* start so a recycled recorded PID
    # is skipped.
...
def snapshot_targets(state: DevState, deps: DevDeps) -> TargetSet:
    ...
    if pid is not None and deps.killer.is_alive(pid):
        if (
            state.arbiter_start_time is not None
            and not deps.killer.identity_matches(pid, state.arbiter_start_time)
        ):
    ...
    if state.frontend_pid is not None and state.frontend_start_time is not None:
        watcher_identities.append(
            (state.frontend_pid, state.frontend_start_time)
        )
    return TargetSet(refused, arbiter_alive, descendants, watcher_identities)


def teardown(
    state: DevState | None, deps: DevDeps, *, lock_held: bool
) -> StopResult:
    ...
    if targets.refused:
        ...
        result = StopResult("refused", pid=state.arbiter_pid, message=message)
        # Pass the local `message` (str), not result.message (str | None).
        log_diagnostic(deps, message)
        return result  # keep dev-state + sockets + pidfile

    result = kill_arbiter(state, targets, deps)
    if result.kind == "survivor":
        remove_artifacts(
            state, deps, result, lock_held=lock_held
        )  # keeps sockets
        return result
...
def kill_arbiter(
    state: DevState, targets: TargetSet, deps: DevDeps
) -> StopResult:
    """Quit the arbiter, confirm death, escalate to a gated kill if needed."""
    statuses = _probe_status(state, deps, timeout=deps.probe_timeout)
    reachable = statuses is not None
    if reachable:
        sup = deps.client_factory(state.endpoint, timeout=deps.probe_timeout)
        try:
            # circus: SIGTERM -> 2 s graceful_timeout -> SIGKILL w/
            # stop_children
            sup.quit()
        except SupervisorUnreachableError as exc:
    ...
    # Confirm death, generous enough to cover circus's own 2 s graceful
    # teardown.
    ...
    if _wait_until_dead(state.arbiter_pid, deps, deps.grace_kill):
        return StopResult("clean")
    msg = (
        f"arbiter {state.arbiter_pid} still alive after SIGKILL; left "
        f"dev-state + sockets in place — see {deps.diagnostic_log}, then "
        f"re-run `mise run dev:stop`"
    )
    log_diagnostic(deps, msg)
...
def reap_descendants(
    ...
    Combines the pre-quit snapshot (catches the in-tree descendants) with a
    fresh walk of the recorded ``server_pid``/``frontend_pid`` (catches
    workers spawned lazily during teardown and orphans reparented away from a
    dead arbiter).
    """
    baseline: dict[int, float] = dict(targets.descendants)
...
def do_restart(deps: DevDeps) -> UpResult:
    ...
    return UpResult(
        ...
        message=(
            result.message
            or f"could not stop the existing stack ({result.kind}); "
            "investigate and re-run `mise run dev:stop`, then `mise run dev`"
        ),
        artifact=str(deps.diagnostic_log),
    )
