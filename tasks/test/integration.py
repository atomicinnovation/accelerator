...
# retired in favour of `accelerator corpus`. Dropped to 15 as the meta-directory
# migration engine's interactive wire-protocol harness
# (test-interactive-protocol.sh) retired in favour of the native
# accelerator-migrate port.
...
# holds only the one bash harness that predates ADR-0048 (test-vcs-detect.sh
# — the meta-directory migration discoverability hook's own harness retired
# alongside the bash migration engine it gated); the link-refresh suite is
# pytest, where a lost file is a collection error rather than a silently
# smaller run, so no by-name entry is needed.
